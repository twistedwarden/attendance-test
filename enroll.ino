...
const char *ssid2 = "ASHBORN_2.4G";
const char *pass2 = "Ww2TWKy2";
const char *ssid3 = "TestWifi";
const char *pass3 = "12345678";
const char *ssid4 = "FOOTHILLS";
const char *pass4 = "FCSQC123";
const char *ssid5 = "RANDOM";
const char *pass5 = "RANDOM123";

// Fingerprint API (Node) endpoint base (Railway public URL over HTTPS)
String apiUrl = "https://foothills.up.railway.app/api/fingerprint/verify-id";         // Fingerprint verification endpoint
String healthUrl = "https://foothills.up.railway.app/api/fingerprint/health";         // Health check endpoint
String controlApiUrl = "https://foothills.up.railway.app/api/esp32/devices/ESP32-01"; // ESP32 control API
...
{
  if (!displayReady)
    return; // Guard against calls before begin()
  ...
  if (displayDriver == 1)
  {
    ...
    else
    {
      ...
      if (line2.length())
        u8g2_ssd.drawStr(0, 30, line2.c_str());
      if (line3.length())
        u8g2_ssd.drawStr(0, 45, line3.c_str());
    }
  ...
  else if (displayDriver == 2)
  {
    ...
    else
    {
      ...
      if (line2.length())
        u8g2_sh.drawStr(0, 30, line2.c_str());
      if (line3.length())
        u8g2_sh.drawStr(0, 45, line3.c_str());
    }
    u8g2_sh.sendBuffer();
  }
}
  ...
  if (httpCode == 200)
  {
    ...
    if (response.indexOf("\"hasCommand\":true") > 0)
    {
      ...
      else if (response.indexOf("\"command\":\"enroll\"") > 0)
      {
        ...
        if (studentIdEnd == -1)
          studentIdEnd = response.indexOf("}", studentIdStart);
...
void sendAttendance(int fingerId)
{
  ...
  else
  {
    ...
    if (probe.begin(secureClient, healthUrl))
    {
      ...
      int hc = probe.GET();
      Serial.print("Health GET: ");
      Serial.println(hc);
      if (hc > 0)
        Serial.println(probe.getString());
      probe.end();
    }
